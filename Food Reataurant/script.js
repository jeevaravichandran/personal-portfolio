...
search.addEventListener("keyup", function () {
    ...
    for (count = 0; count < items.length; count++) {
        var itemlist = items[count].querySelector("h3").textContent

        if (itemlist.toUpperCase().indexOf(enteredText) >= 0) {
            items[count].style.display = "block"
            document.querySelector(".unavail").style.display="none" 
        }
        else {
            items[count].style.display = "none"
            document.querySelector(".unavail").style.display="block"
        }
    }
})
...
function formvalidation(event) {
    ...
    // Form Validation and Show Popup Layer and Popup message
        ...
        setTimeout(function(){ 
            document.querySelector(".overlay-response").style.display = "block"
